...
const WeatherWidget = () => {
    ...
    const handleSearch = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            let key = process.env.NEXT_PUBLIC_WEATHER_API_KEY;
          const response = await fetch(`https://api.weatherapi.com/v1/current.json?key=${key}&q=${trimmedLocation}`);
          if (!response.ok) {
      ...
      return (
        <div className="flex justify-center bg-blue-300 items-center h-screen">
          <Card className="w-full max-w-md mx-auto bg-yellow-200 text-center">
            <CardHeader>
